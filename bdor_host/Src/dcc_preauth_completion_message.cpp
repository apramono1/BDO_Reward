...
static const std::vector<int> kDccPreAuthCompletionRspMandatoryFields = {
...
static bool ValidateDccPreAuthCompletionResponse(const iso8583::Apdu& request_apdu,
                                    const iso8583::Apdu& response_apdu);

// PUBLIC FUNCTIONS

iso8583::Apdu BuildPreAuthCompletionRequestWithDccEnquiry(const Transaction& tx) {
  auto request_apdu = BuildPreAuthCompletionRequest(tx);
...
bool ReadAndValidatePreAuthCompletionResponseWithDccEnquiry(
  ...
  if (!ValidateDccPreAuthCompletionResponse(request_apdu, response_apdu))
    return false;
...
iso8583::Apdu BuildPreAuthCompletionRequestWithDccAllowed(const Transaction& tx) {
  iso8583::Apdu request_apdu = BuildPreAuthCompletionRequest(tx);
...
static bool ValidateDccPreAuthCompletionResponse(const iso8583::Apdu& request_apdu,
                                    const iso8583::Apdu& response_apdu) {
  if (!CheckMandatoryFields(response_apdu, kDccPreAuthCompletionRspMandatoryFields)
