...
static const std::vector<int> kDccSaleRspMandatoryFields = {
...
static bool ValidateDccSaleResponse(const iso8583::Apdu& request_apdu,
                                    const iso8583::Apdu& response_apdu);

// PUBLIC FUNCTIONS

iso8583::Apdu BuildSaleRequestWithDccEnquiry(const Transaction& tx) {
  auto request_apdu = BuildSaleRequest(tx);
...
bool ReadAndValidateSaleResponseWithDccEnquiry(
  ...
  if (!ValidateDccSaleResponse(request_apdu, response_apdu))
    return false;

  auto field_content = GetDccResponseField(response_apdu);
  if (!ReadDccEnquiryResponseData(field_content, tx)) {
    return false;
  }
...
iso8583::Apdu BuildSaleRequestWithDccAllowed(const Transaction& tx) {
...
bool ReadAndValidateSaleResponseWithDccAllowed(
    const iso8583::Apdu& request_apdu,
    const std::vector<uint8_t>& data,
    Transaction& tx) {
  iso8583::Apdu response_apdu(bdor_spec(), data.data(),
...
static bool ValidateDccSaleResponse(const iso8583::Apdu& request_apdu,
                                    const iso8583::Apdu& response_apdu) {
  if (!CheckMandatoryFields(response_apdu, kDccSaleRspMandatoryFields)
