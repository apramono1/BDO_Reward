...
static const std::vector<int> kDccPreAuthRspMandatoryFields = {
...
static bool ValidateDccPreAuthResponse(const iso8583::Apdu& request_apdu,
                                    const iso8583::Apdu& response_apdu);

// PUBLIC FUNCTIONS

iso8583::Apdu BuildPreAuthRequestWithDccEnquiry(const Transaction& tx) {
  auto request_apdu = BuildPreAuthRequest(tx);
...
bool ReadAndValidatePreAuthResponseWithDccEnquiry(
    const iso8583::Apdu& request_apdu,
    const std::vector<uint8_t>& data,
    Transaction& tx) {

  iso8583::Apdu response_apdu = iso8583::Apdu(bdor_spec(), data.data(),
                              data.size());


  logger::debug(iso8583::Print(response_apdu).c_str());

  if (!ValidateDccPreAuthResponse(request_apdu, response_apdu))
    return false;
...
iso8583::Apdu BuildPreAuthRequestWithDccAllowed(const Transaction& tx) {
  iso8583::Apdu request_apdu = BuildPreAuthRequest(tx);
...
bool ReadAndValidatePreAuthResponseWithDccAllowed(
    const iso8583::Apdu& request_apdu,
    const std::vector<uint8_t>& data,
    Transaction& tx) {
...
static bool ValidateDccPreAuthResponse(const iso8583::Apdu& request_apdu,
                                       const iso8583::Apdu& response_apdu) {
  if (!CheckMandatoryFields(response_apdu, kDccPreAuthRspMandatoryFields)
