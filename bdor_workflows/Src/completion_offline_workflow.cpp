...
CompletionOfflineWorkflow::CompletionOfflineWorkflow(
...
void CompletionOfflineWorkflow::Start(
  ...
  if (ProcessCompletionSaleOffline(tx) == TransactionStatus::APPROVED) {
...
WorkflowStatus CompletionOfflineWorkflow::RunManualFlow(Transaction& tx) {
...
WorkflowStatus CompletionOfflineWorkflow::RunMagFlow(
...
WorkflowStatus CompletionOfflineWorkflow::RunEmvFlow(Transaction& tx) {
...
WorkflowStatus CompletionOfflineWorkflow::RunClessFlow(
...
TransactionStatus CompletionOfflineWorkflow::ProcessCompletionSaleOffline(
    Transaction& tx) {
  ...
  if (!RunDccFlow(app_settings_, ui_, batch_, host_,
                  tx,
                  &HostSwitch::PerformOfflineWithDccEnquiry,
                  &HostSwitch::PerformOfflineWithDccAllowed,
                  host_status)) {
...
TransactionStatus CompletionOfflineWorkflow::ProcessPostOnlineNonFullEmv(
...
bool CompletionOfflineWorkflow::IsTxnEnabledPerCard(
  ...
  if (!card_config->tx_preauth_completion_offline_enabled) {
...
bool CompletionOfflineWorkflow::VerifyPreAuthCompletionAmount(
...
bool CompletionOfflineWorkflow::RequestCompletionAmount(Transaction& tx) {
...
void CompletionOfflineWorkflow::RunPrintReceiptFlow(
    const Transaction& tx) const {
