...
RewardWorkflow::RewardWorkflow(ApplicationSettings& app_settings,
                                 Batch& batch,
                                 HostSwitch& host,
								 Acquirer& acquirer,
...
void RewardWorkflow::Start(types::PosConditionCode pos_condition_mode) {

  logger::debug("Starting BDO Reward Transaction");
  ...
  if (!card_presenter_.PromptCardToStart(this, tx)) {
    host_.Disconnect();
...
WorkflowStatus RewardWorkflow::RunManualFlow(Transaction& tx) {
...
WorkflowStatus RewardWorkflow::RunMagFlow(types::MagStripe mag_stripe,
                                           Transaction& tx) {
...
WorkflowStatus RewardWorkflow::RunEmvFlow(Transaction& tx) {
  ...
  host_.ValidateCard(tx);
  ProcessPostCardEMVValidation(tx, emv_workflow, batch_, ui_, acquirer_, host_);
  return WorkflowStatus::EMV_SUCCESS;
}

WorkflowStatus RewardWorkflow::RunClessFlow(
