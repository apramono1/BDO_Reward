...
PreauthWorkflow::PreauthWorkflow(ApplicationSettings& app_settings,
                                 Batch& batch,
                                 HostSwitch& host,
...
void PreauthWorkflow::Start(types::PosConditionCode pos_condition_mode) {
  logger::debug("Starting Pre-auth Transaction");
  ...
  Transaction tx;
  tx.transaction_type = TransactionType::PREAUTH;
...
Transaction PreauthWorkflow::Start(const types::Amount& amount,
  ...
  tx.preauth_amount = amount;
  if (!IsValidAmount(app_settings_, *tx.GetTotalPreauthAmount())) {
...
WorkflowStatus PreauthWorkflow::RunManualFlow(Transaction& tx) {
  ...
  if (!RunDccFlow(app_settings_, ui_, batch_, host_,
                  tx,
                  &HostSwitch::AuthorizePreAuthWithDccEnquiry,
                  &HostSwitch::AuthorizePreAuthWithDccAllowed,
                  host_status)) {
    return WorkflowStatus::MANUAL_TIMEOUT_OR_CANCEL;
  }
...
WorkflowStatus PreauthWorkflow::RunMagFlow(types::MagStripe mag_stripe,
                                           Transaction& tx) {
...
WorkflowStatus PreauthWorkflow::RunEmvFlow(Transaction& tx) {
...
WorkflowStatus PreauthWorkflow::RunClessFlow(
