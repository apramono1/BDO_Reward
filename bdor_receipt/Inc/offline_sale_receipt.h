/*
 ...
 ------------------------------------------------------------------------------
 */
#ifndef BDOR__OFFLINE_SALE_RECEIPT_H_
#define BDOR__OFFLINE_SALE_RECEIPT_H_
...
class OfflineSaleReceipt : public receipt::GoalCompositeReceipt {

  struct Body : receipt::GoalReceipt {
    Body(tpcore::GoalHandle& gl,
         unsigned printer_timeout,
		 TerminalConfiguration::TipProcessingMode tip_mode = TerminalConfiguration::TIP_DISABLED);
    void Expand(const Receipt::Map& dict) override;
    TerminalConfiguration::TipProcessingMode tip_mode;
  };

 public:
  OfflineSaleReceipt(tpcore::GoalHandle& gl,
                     unsigned printer_timeout,
                     CopyType copy,
                     ReprintType reprint,
