...
PreauthReceipt::PreauthReceipt(tpcore::GoalHandle& gl,
  ...
  Add(stdx::make_unique<ReceiptCVM>(gl_, printer_timeout, cvm_type));
  Add(stdx::make_unique<ReceiptFooter>(gl_, printer_timeout, copy, reprint));

}

PreauthReceipt::Body::Body(tpcore::GoalHandle& gl,
                           unsigned printer_timeout)
    : GoalReceipt(gl, printer_timeout){
}

void PreauthReceipt::Body::Expand(const Receipt::Map& dict) {
